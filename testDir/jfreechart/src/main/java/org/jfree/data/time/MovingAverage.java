...
public class MovingAverage {
     ...
     */
    public static <S extends Comparable<S>> TimeSeries<S> createMovingAverage(TimeSeries<S> source, S name, int periodCount, int skip) {
        Args.nullNotPermitted(source, "source");
        if (periodCount < 1) {
            throw new IllegalArgumentException("periodCount must be greater " + "than or equal to 1.");
        }
     ...
     * @return The moving average series.
     */
    public static <S extends Comparable<S>> TimeSeries<S> createPointMovingAverage(TimeSeries<S> source, S name, int pointCount) {
        Args.nullNotPermitted(source, "source");
        if (pointCount < 2) {

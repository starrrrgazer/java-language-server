...
public class PaintAlpha {
    ...
    private static Color darker(Color paint) {
        return new Color((int) (paint.getRed() * FACTOR), (int) (paint.getGreen() * FACTOR), (int) (paint.getBlue() * FACTOR), paint.getAlpha());
    }
    ...
    private static TexturePaint darkerTexturePaint(TexturePaint paint) {
        ...
        if (img.getColorModel() instanceof IndexColorModel) {
            // RGB (+ optional Alpha which we leave
            int[] nco = new int[4];
            // unchanged)
            for (int y = miY; y < maY; y++) {
                ...
                for (int p = 0; p < pix.length; p++) {
                    ...
                    // Green
                    nco[1] *= FACTOR;
                    // Blue. Now map computed colour to
                    nco[2] *= FACTOR;
                    // nearest in Palette...
